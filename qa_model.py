...
from modules import RNNEncoder, SimpleSoftmaxLayer, BasicAttn, GateAttnCell, SelfAttn, Output_Layer
...
class QAModel(object):
    ...
    def build_graph(self):
        ...
        if self.FLAGS.attention == "Rnet":
            #Get context and question hidden states based on bidirectional gru
            with vs.variable_scope("Question_Context_RNNEncoder"):
                encoder = RNNEncoder(self.FLAGS.hidden_size, self.keep_prob)
                context_hiddens = encoder.build_graph(self.context_embs, self.context_mask) # (batch_size, context_len, hidden_size*2)
                question_hiddens = encoder.build_graph(self.qn_embs, self.qn_mask) # (batch_size, question_len, hidden_size*2)

            ### Step: Gated Attention-Based RNN (if include character embedding, self.FLAGS.hidden_size*2 for content & question hidden_size)
            gated_attn_cell_fw = GateAttnCell(self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size, question_hiddens) 
            gated_attn_cell_bw = GateAttnCell(self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size, question_hiddens)
            
            #gated_outputs (output_fw, output_bw) -- each [batch_size, max_time, cell_fw.output_size]
            gated_outputs , _ = tf.nn.bidirectional_dynamic_rnn(gated_attn_cell_fw, gated_attn_cell_bw, inputs=context_hiddens, dtype=tf.float32)

            #Apply dropout to every time step of the gated_outputs
            gated_outputs_dropout = tf.nn.dropout(tf.concat(gated_outputs, 2), self.keep_prob) #(batch_size, context_len, hidden_size*2)

            ### Step: Self Attention Layer
            SelfAttn_layer  = SelfAttn(self.FLAGS.context_len, self.FLAGS.hidden_size * 2, self.FLAGS.hidden_size)
            _ , c_t = SelfAttn_layer.build_graph(gated_outputs_dropout , self.context_mask)

            #BiRNN: BiRNN(h_(t-1)P, [v_tP, c_t])
            with vs.variable_scope("SelfAttn_RNNEncoder"):
                SelfAttn_BiRNN = RNNEncoder(self.FLAGS.hidden_size, self.keep_prob)
                SelfAttn_outputs = SelfAttn_BiRNN.build_graph(c_t, self.context_mask) #(batch_size, context_len, hidden_size*2)

            ### Step: Output Layer
            output_layer = Output_Layer(self.FLAGS.hidden_size*2, self.FLAGS.question_len, self.FLAGS.context_len, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size)
            self.logits_start, self.logits_end = output_layer.build_graph(question_hiddens, SelfAttn_outputs, self.context_mask, self.qn_mask)
